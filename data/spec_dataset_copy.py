...
def python_load_spectrogram(file_path_tensor):
    ...
    try:
        ...
        return spec, label_vec, yt_id
        
    except Exception as e:
        print(f"Error loading {file_path}: {e}")
        # Return dummy zero spectrogram
        return np.zeros((IMAGE_HEIGHT, IMAGE_WIDTH, NUM_CHANNELS), dtype=np.float32), np.zeros((AUDIOSET_NUM_CLASSES,), dtype=np.float32), "error"

def load_spectrogram_dataset(file_path):
    """
    Wraps the python function and returns a Dataset.
    """
    [spec, label, yt_id] = tf.py_function(
    ...
    return tf.data.Dataset.from_tensors((spec, label, yt_id))

# ==============================================================================
# 4. View Generation (Augmentation)
# ==============================================================================
def generate_views(spectrogram, label, yt_id):
    """
    Takes one full spectrogram and generates V views.
    Input: (H, W, C), (NUM_CLASSES,), ()
    Output: (V, H, W, C), (V, NUM_CLASSES), (V,)
    """
    # clean NaN
    ...
    yt_id = tf.expand_dims(yt_id, axis=0)  # (1,)
    return tf.stack(views), tf.tile(label, [NUM_GLOBAL_VIEWS + NUM_LOCAL_VIEWS, 1]), tf.tile(yt_id, [NUM_GLOBAL_VIEWS + NUM_LOCAL_VIEWS]) # (V, NUM_CLASSES), (V,)
...
def random_drop_channels(spec, prob=0.1):
    ...
    def drop_channel():
        ...
        return tf.cond(
            channel_to_drop == 0,
            lambda: tf.concat([tf.zeros_like(spec[:, :, 0:1]), spec[:, :, 1:2]], axis=2),
            lambda: tf.concat([spec[:, :, 0:1], tf.zeros_like(spec[:, :, 1:2])], axis=2)
        )

    return tf.cond(drop, drop_channel, lambda: spec)
...
# 5. Dataset Pipeline
# ==============================================================================
def preprocess_batch(spec_batch, label_batch, yt_id_batch, training=True):
    """
    ...
    if training:
        ...
        views, labels, yt_ids = tf.map_fn(
            lambda x: generate_views(x[0], x[1], x[2]),
            (spec_batch, label_batch, yt_id_batch),
            fn_output_signature=(tf.float32, tf.float32, tf.string),
            parallel_iterations=32
        )
    else:
        # Expand dims for eval
        def expand_view(spec, label, yt_id):
            spec = tf.expand_dims(spec, axis=0) # (1, H, W, C)
            label = tf.expand_dims(label, axis=0) # (1, NUM_CLASSES)
            yt_id = tf.expand_dims(yt_id, axis=0) # (1,)
            return spec, label, yt_id
            
        views, labels, yt_ids = tf.map_fn(
            lambda x: expand_view(x[0], x[1], x[2]),
            (spec_batch, label_batch, yt_id_batch),
            fn_output_signature=(tf.float32, tf.float32, tf.string),
            parallel_iterations=32
        )

    return views, labels, yt_ids
...
def get_dataset(data_dir, csv_path, batch_size, dataset="audioset", training=True):
    ...
    if dataset == "audioset":
        ...
        gpu_map_func = lambda x, y, z: preprocess_batch(x, y, z, training=training)
...
def visualize_sample(data_dir, csv_path):
    ...
    # Take 1 batch
    for batch, label, yt_ids in ds.take(1):
        # batch shape: (1, TOTAL_VIEWS, H, W, C)
        # label shape: (1, TOTAL_VIEWS, NUM_CLASSES)
        # yt_ids shape: (1, TOTAL_VIEWS)
        
        # --- Verify Label Consistency ---
        print("Verifying label consistency across views...")
        batch_size_val = batch.shape[0]
        num_views_val = batch.shape[1]
        
        for b in range(batch_size_val):
            first_label = label[b, 0, :].numpy()
            sample_yt_id = yt_ids[b, 0].numpy().decode('utf-8')
            print(f"Sample {b} YTID: {sample_yt_id}")
            
            for v in range(1, num_views_val):
                current_label = label[b, v, :].numpy()
                if not np.array_equal(first_label, current_label):
                    print(f"CRITICAL ERROR: Label mismatch in sample {b}, view {v} vs view 0")
                    print(f"View 0: {first_label}")
                    print(f"View {v}: {current_label}")
                else:
                    pass # Consistent
            print(f"Sample {b} labels consistent across {num_views_val} views.")
        # --------------------------------
        ...
        yt_id_str = yt_ids[0, 0].numpy().decode('utf-8') # Get YTID
        ...
        # --- MODIFIED TITLE ---
        fig.suptitle(f"YTID: {yt_id_str}\nLabels: {', '.join(label_names)}", fontsize=12)
        # ----------------------

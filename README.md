...
*   Invariance Loss: $L_{inv} = \mathbb{E}*{x \sim \mathcal{D}}; \mathbb{E}*{v \sim \mathcal{V}(x)} \left[ \left| z_{g}^{avg}(x) - z_v(x) \right|_2^2 \right]$ where $z_{g}^{avg}(x) =
\frac{1}{G} \sum_{i=1}^{G} z_{g_i}(x)$ is the average embedding of all global views of input $x$, and $z_v(x)$ is the embedding of view $v$ of input $x$.
